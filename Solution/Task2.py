...
def load_data_with_individual_digits(directory, flatten=True, max_samples=None):
    ...
    # Loop through each label directory with progress bar
    sample_count = 0
...
# Calculate overall sequence accuracy for decision tree
...
# Define a CNN model for digit recognition
...
# Train a separate CNN for each digit position
cnn_models = []
cnn_accuracies = []
...
for digit_position, (y_train_digit, y_val_digit, y_test_digit) in enumerate([
    ...
    plt.tight_layout()
    plt.savefig(f'cnn_learning_curves_digit{digit_position+1}.png')
    plt.show()

# Calculate overall sequence accuracy for CNN
...
plt.tight_layout()
plt.savefig('per_digit_accuracy.png')
plt.show()
...
# Create confusion matrices for the most interesting digit position
# First digit (can change to 1 or 2 for other positions)
digit_pos = 0
