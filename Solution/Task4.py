...
# Split an image into three equal parts vertically
def split_image(img):
    w = img.shape[1] // 3
...
X_data = [
    [X_train_p1, X_train_p2, X_train_p3],
...
# Train 3 small baseline CNNs on subsets of each digit’s data
print("\n- TRAINING BASELINE MODELS")
digit_names = ["First", "Second", "Third"]
baseline_histories = []
for i, (X_train, y_train_digit, X_val, y_val_digit) in enumerate(
    zip(X_data[0], y_train_digits, X_data[1], y_val_digits)
):
    ...
    history = model.fit(
        ...
        epochs=5,
        batch_size=64,
        validation_data=(X_val[:1000], y_val_digit[:1000]),
        verbose=1,
    )
...
# Evaluate models on test data and compute per-digit and sequence accuracy
print("- EVALUATING ENHANCED MODELS")
preds = [
    np.argmax(model.predict(X_data[2][i], verbose=0), axis=1)
...
# Plot learning curves (accuracy/loss) for each digit model
...
# - COMPARISON PLOTS
# Compare per-digit accuracy with baseline from Task 3
baseline_accs = [0.993, 0.101, 0.087]
x = np.arange(3)
width = 0.35
...
for i in range(3):
    plt.text(
        i - width / 2, baseline_accs[i] + 0.01, f"{baseline_accs[i]:.3f}", ha="center"
    )
    plt.text(i + width / 2, accs[i] + 0.01, f"{accs[i]:.3f}", ha="center")
save_figure("per_digit_accuracy_comparison.png")

# Compare sequence accuracy from Task 3 and Task 4
...
# - FINAL RESULTS
# Print per-digit and sequence accuracy results
print("\n- FINAL RESULTS")
