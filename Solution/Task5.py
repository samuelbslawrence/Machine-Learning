...
def load_images_for_gan(directory, max_samples=5000):
    ...
    for label_idx, label in enumerate(label_dirs[:50]):
        ...
        for img_idx, img_path in enumerate(
            glob.glob(os.path.join(img_dir, "*.png"))[:50]
        ):
...
X_train_full, y_train_full, train_label_strings = load_images_for_classifier(
    train_dir, max_samples=10000
)
X_test, y_test, test_label_strings = load_images_for_classifier(
    test_dir, max_samples=2000
)
...
sorted_distribution = dict(
    sorted(label_distribution.items(), key=lambda x: x[1], reverse=True)
)
...
if not load_pretrained:
    ...
    for epoch in range(EPOCHS):
        ...
        num_batches = len(X_train_gan) // BATCH_SIZE + (
            1 if len(X_train_gan) % BATCH_SIZE != 0 else 0
        )
        batch_progbar = Progbar(num_batches, stateful_metrics=["gen_loss", "disc_loss"])
        ...
        for batch in train_dataset:
            ...
            batch_count += 1
            # Update batch progress bar with epoch-level metrics (final values)
...
plt.hist(
    real_means, bins=30, alpha=0.7, label="Real Images", density=True, color="blue"
)
plt.hist(
    synthetic_means,
    bins=30,
...
y_train = [
    y_train_full[0][:train_size],
    y_train_full[1][:train_size],
    y_train_full[2][:train_size],
]
y_val = [
    y_train_full[0][train_size:],
    y_train_full[1][train_size:],
...
# Create datasets
train_dataset_classifier = tf.data.Dataset.from_tensor_slices(
    (X_train, (y_train[0], y_train[1], y_train[2]))
)
train_dataset_classifier = train_dataset_classifier.shuffle(len(X_train)).batch(
    CLASSIFIER_BATCH_SIZE
)

val_dataset_classifier = tf.data.Dataset.from_tensor_slices(
    (X_val, (y_val[0], y_val[1], y_val[2]))
)
val_dataset_classifier = val_dataset_classifier.batch(CLASSIFIER_BATCH_SIZE)
...
for epoch in range(CLASSIFIER_EPOCHS):
    ...
    # Training phase
    num_train_batches = len(X_train) // CLASSIFIER_BATCH_SIZE + (
        1 if len(X_train) % CLASSIFIER_BATCH_SIZE != 0 else 0
    )
    batch_progbar = Progbar(
        num_train_batches, stateful_metrics=["acc1", "acc2", "acc3"]
    )
    ...
    batch_count = 0
    for batch_x, (batch_y1, batch_y2, batch_y3) in train_dataset_classifier:
        with tf.GradientTape() as tape:
            predictions = classifier_baseline(batch_x, training=True)
            pred1, pred2, pred3 = predictions

            loss1 = tf.keras.losses.categorical_crossentropy(batch_y1, pred1)
            loss2 = tf.keras.losses.categorical_crossentropy(batch_y2, pred2)
            loss3 = tf.keras.losses.categorical_crossentropy(batch_y3, pred3)
            total_loss = tf.reduce_mean(loss1 + loss2 + loss3)

        gradients = tape.gradient(total_loss, classifier_baseline.trainable_variables)
        classifier_baseline.optimizer.apply_gradients(
            zip(gradients, classifier_baseline.trainable_variables)
        )

        # Calculate accuracies
        acc1 = tf.reduce_mean(
            tf.cast(
                tf.equal(tf.argmax(batch_y1, axis=1), tf.argmax(pred1, axis=1)),
                tf.float32,
            )
        )
        acc2 = tf.reduce_mean(
            tf.cast(
                tf.equal(tf.argmax(batch_y2, axis=1), tf.argmax(pred2, axis=1)),
                tf.float32,
            )
        )
        acc3 = tf.reduce_mean(
            tf.cast(
                tf.equal(tf.argmax(batch_y3, axis=1), tf.argmax(pred3, axis=1)),
                tf.float32,
            )
        )

        epoch_losses.append(total_loss.numpy())
        epoch_digit1_losses.append(tf.reduce_mean(loss1).numpy())
        epoch_digit2_losses.append(tf.reduce_mean(loss2).numpy())
        epoch_digit3_losses.append(tf.reduce_mean(loss3).numpy())
        epoch_digit1_acc.append(acc1.numpy())
        epoch_digit2_acc.append(acc2.numpy())
        epoch_digit3_acc.append(acc3.numpy())

        batch_count += 1
        batch_progbar.update(
            batch_count,
            values=[
                ("acc1", np.mean(epoch_digit1_acc)),
                ("acc2", np.mean(epoch_digit2_acc)),
                ("acc3", np.mean(epoch_digit3_acc)),
            ],
        )

    # Validation phase (computed after training, no progress bar)
    val_losses = []
    val_digit1_losses = []
    val_digit2_losses = []
    val_digit3_losses = []
    ...
    for batch_x, (batch_y1, batch_y2, batch_y3) in val_dataset_classifier:
        predictions = classifier_baseline(batch_x, training=False)
        pred1, pred2, pred3 = predictions
        ...
        val_digit1_losses.append(tf.reduce_mean(loss1).numpy())
        val_digit2_losses.append(tf.reduce_mean(loss2).numpy())
        val_digit3_losses.append(tf.reduce_mean(loss3).numpy())
        val_digit1_acc.append(acc1.numpy())
        val_digit2_acc.append(acc2.numpy())
        val_digit3_acc.append(acc3.numpy())

    # Record epoch metrics
    avg_train_loss = np.mean(epoch_losses)
    avg_val_loss = np.mean(val_losses)

    baseline_history["digit1_accuracy"].append(np.mean(epoch_digit1_acc))
    baseline_history["digit2_accuracy"].append(np.mean(epoch_digit2_acc))
    baseline_history["digit3_accuracy"].append(np.mean(epoch_digit3_acc))
    baseline_history["digit1_loss"].append(np.mean(epoch_digit1_losses))
    baseline_history["digit2_loss"].append(np.mean(epoch_digit2_losses))
    baseline_history["digit3_loss"].append(np.mean(epoch_digit3_losses))
    baseline_history["loss"].append(avg_train_loss)

    baseline_history["val_digit1_accuracy"].append(np.mean(val_digit1_acc))
    baseline_history["val_digit2_accuracy"].append(np.mean(val_digit2_acc))
    baseline_history["val_digit3_accuracy"].append(np.mean(val_digit3_acc))
    baseline_history["val_digit1_loss"].append(np.mean(val_digit1_losses))
    baseline_history["val_digit2_loss"].append(np.mean(val_digit2_losses))
    baseline_history["val_digit3_loss"].append(np.mean(val_digit3_losses))
    baseline_history["val_loss"].append(avg_val_loss)

    # Early stopping logic
    if avg_val_loss < best_val_loss:
        best_val_loss = avg_val_loss
        patience_counter = 0
        best_weights = classifier_baseline.get_weights()
    else:
        patience_counter += 1
        if patience_counter >= 5:  # patience=5
            print(f"\nEarly stopping at epoch {epoch + 1}")
            classifier_baseline.set_weights(best_weights)
            break

    # Learning rate reduction logic (simplified)
    if patience_counter >= 3:  # patience=3 for LR reduction
        current_lr = classifier_baseline.optimizer.learning_rate.numpy()
        new_lr = current_lr * 0.2
        if new_lr >= 0.00001:  # min_lr
            classifier_baseline.optimizer.learning_rate.assign(new_lr)
...
baseline_scores = classifier_baseline.evaluate(X_test, y_test, verbose=0)
...
baseline_avg_accuracy = (
    baseline_scores[4] + baseline_scores[5] + baseline_scores[6]
) / 3
...
for i in range(num_pred_batches):
    ...
    batch_pred = classifier_baseline.predict(
        synthetic_images_normalized[start_idx:end_idx], verbose=0
    )
...
print(
    f"Kept {len(synthetic_filtered)} synthetic images with confidence > {confidence_threshold}"
)
...
train_dataset_augmented = tf.data.Dataset.from_tensor_slices(
    (X_augmented, (y_augmented[0], y_augmented[1], y_augmented[2]))
)
train_dataset_augmented = train_dataset_augmented.shuffle(len(X_augmented)).batch(
    CLASSIFIER_BATCH_SIZE
)
...
for epoch in range(CLASSIFIER_EPOCHS):
    ...
    # Training phase
    num_train_batches = len(X_augmented) // CLASSIFIER_BATCH_SIZE + (
        1 if len(X_augmented) % CLASSIFIER_BATCH_SIZE != 0 else 0
    )
    batch_progbar = Progbar(
        num_train_batches, stateful_metrics=["acc1", "acc2", "acc3"]
    )
    ...
    batch_count = 0
    for batch_x, (batch_y1, batch_y2, batch_y3) in train_dataset_augmented:
        with tf.GradientTape() as tape:
            predictions = classifier_augmented(batch_x, training=True)
            pred1, pred2, pred3 = predictions
        ...
        gradients = tape.gradient(total_loss, classifier_augmented.trainable_variables)
        classifier_augmented.optimizer.apply_gradients(
            zip(gradients, classifier_augmented.trainable_variables)
        )
    ...
    for batch_x, (batch_y1, batch_y2, batch_y3) in val_dataset_classifier:
        predictions = classifier_augmented(batch_x, training=False)
        pred1, pred2, pred3 = predictions
    ...
    augmented_history["digit1_accuracy"].append(np.mean(epoch_digit1_acc))
    augmented_history["digit2_accuracy"].append(np.mean(epoch_digit2_acc))
    augmented_history["digit3_accuracy"].append(np.mean(epoch_digit3_acc))
    augmented_history["digit1_loss"].append(np.mean(epoch_digit1_losses))
    augmented_history["digit2_loss"].append(np.mean(epoch_digit2_losses))
    augmented_history["digit3_loss"].append(np.mean(epoch_digit3_losses))
    augmented_history["loss"].append(avg_train_loss)

    augmented_history["val_digit1_accuracy"].append(np.mean(val_digit1_acc))
    augmented_history["val_digit2_accuracy"].append(np.mean(val_digit2_acc))
    augmented_history["val_digit3_accuracy"].append(np.mean(val_digit3_acc))
    augmented_history["val_digit1_loss"].append(np.mean(val_digit1_losses))
    augmented_history["val_digit2_loss"].append(np.mean(val_digit2_losses))
    augmented_history["val_digit3_loss"].append(np.mean(val_digit3_losses))
    augmented_history["val_loss"].append(avg_val_loss)

    # Early stopping logic
    if avg_val_loss < best_val_loss:
        best_val_loss = avg_val_loss
        patience_counter = 0
        best_weights = classifier_augmented.get_weights()
    else:
        patience_counter += 1
        if patience_counter >= 5:  # patience=5
            print(f"\nEarly stopping at epoch {epoch + 1}")
            classifier_augmented.set_weights(best_weights)
            break

    # Learning rate reduction logic (simplified)
    if patience_counter >= 3:  # patience=3 for LR reduction
        current_lr = classifier_augmented.optimizer.learning_rate.numpy()
        new_lr = current_lr * 0.2
        if new_lr >= 0.00001:  # min_lr
            classifier_augmented.optimizer.learning_rate.assign(new_lr)
...
augmented_scores = classifier_augmented.evaluate(X_test, y_test, verbose=0)
...
augmented_avg_accuracy = (
    augmented_scores[4] + augmented_scores[5] + augmented_scores[6]
) / 3
...
plt.subplot(2, 2, 1)
plt.plot(baseline_history["digit1_accuracy"], label="Baseline Training")
plt.plot(baseline_history["val_digit1_accuracy"], label="Baseline Validation")
plt.plot(augmented_history["digit1_accuracy"], label="Augmented Training")
plt.plot(augmented_history["val_digit1_accuracy"], label="Augmented Validation")
...
plt.subplot(2, 2, 2)
plt.plot(baseline_history["digit2_accuracy"], label="Baseline Training")
plt.plot(baseline_history["val_digit2_accuracy"], label="Baseline Validation")
plt.plot(augmented_history["digit2_accuracy"], label="Augmented Training")
plt.plot(augmented_history["val_digit2_accuracy"], label="Augmented Validation")
...
plt.subplot(2, 2, 3)
plt.plot(baseline_history["digit3_accuracy"], label="Baseline Training")
plt.plot(baseline_history["val_digit3_accuracy"], label="Baseline Validation")
plt.plot(augmented_history["digit3_accuracy"], label="Augmented Training")
plt.plot(augmented_history["val_digit3_accuracy"], label="Augmented Validation")
...
# Loss
plt.subplot(2, 2, 4)
plt.plot(baseline_history["loss"], label="Baseline Training")
plt.plot(baseline_history["val_loss"], label="Baseline Validation")
plt.plot(augmented_history["loss"], label="Augmented Training")
plt.plot(augmented_history["val_loss"], label="Augmented Validation")
...
plt.ylabel("Accuracy")
plt.ylim(
    max(0, min(baseline_avg_accuracy, augmented_avg_accuracy) * 0.9),
